  ...
  return (
    <div className="feature-page">
      <div className="feature-heading">
        <div><span className="panel-kicker">Applicant pipeline</span><h2>Review the waitlist</h2><p>Only explicit approval unlocks invite delivery.</p></div>
      ...
      <div className="filter-bar">
        ...
        <label>Status<select value={status} onChange={(event) => setStatus(event.target.value as StatusFilter)}><option value="all">All statuses</option><option value="pending">Pending</option><option value="approved">Approved</option><option value="invited">Invited</option><option value="rejected">Rejected</option></select></label>
      ...
      {selected.size > 0 && <div className="bulk-bar"><strong>{selected.size} selected</strong><button type="button" className="primary-button" onClick={() => applyTransition([...selected], 'approve')}>Approve</button><button type="button" className="danger-button" onClick={() => applyTransition([...selected], 'reject')}>Reject</button></div>}
